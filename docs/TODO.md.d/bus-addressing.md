...
| 2 | 2026-07-29, mid-build (dictated): "I asked multiple times for subtasks to be displayed again under the stage… I wondered if that was done." | **in progress — model-level PROVEN, screen-level pending.** The required test now exists and passes (`tests/test_sidebar_area_cd.py` `SubagentPlacementTests`, commit 43b104e): subagent rows nest immediately after their owning agent's identity row, beneath the task, never between step rows (Decision-098). Remains a MUST-SHOW acceptance item at E2E: proven done only when visible on his screen. |
