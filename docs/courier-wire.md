...
## 2c. The boxes and the dispatch — scenario inbox-outbox (ruled 2026-08-08)

**[SPEC]** The INBOX is the sole location in which a courier receives ALL
message types; the OUTBOX is where a courier puts ALL outgoing message types —
a sender never writes into another courier's storage. The DELIVERY DISPATCH is
the component that moves messages from the outbox to the recipient inbox(es):
one-to-one, one-to-many, routed, broadcast — every delivery shape is the
dispatch's, never the courier's. The sender knows only the destination
session id; routing knowledge lives exclusively in the dispatch.
...
inotify instances per user on the host, 50 already in use).

**[SPEC]** MESSAGING IS NOT STORAGE — think UDP. A message is deleted as soon
as it is received. Nothing is stored in `.git/`. Permanence is never a message
property. ACK, retry and encryption are out of scope. Message loss during the
rebuild is accepted; no transition compatibility is owed (all new formats,
zero tolerance both ways).
...
The envelope is strict both ways: exactly `from`, `to`, `subject`, optional
`body`; `:session:<dot-free-id>` addresses; subject by exact membership in the
closed 22-subject corpus (Decision-092, carried as the base under review);
any deviation is rejected on send and quarantined on dispatch.
...
**[GAP]** The other delivery shapes (one-to-many, routed, broadcast), the
shared monitor over the inbox, the three-level breach ladder
(questionable/warning/error), the by-reference hand-up, and the cutover of
`courier.py`'s direct-write delivery onto the boxes belong to later
scenarios. `tools/message.schema.json` still describes the OLD envelope; it
becomes the single schema definition for the new wire at the session-messaging
cutover.
---
