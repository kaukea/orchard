...
- `feature · todo · critical · blocked-on-answers · process ·` [The message bus, implemented to its specification — spec built, tested, token-efficient, with the sidebar that consumes it (LAST ATTEMPT)](TODO.md.d/bus-addressing.md) ~transport-test-reconciling ~message-bus ~courier-delegating
- `feature · todo · critical · queued · process ·` [No agent teardown at all: delete landscaper-teardown.sh and @gardener_id — the placement component closes the UI element on stopped (Decision-114)](TODO.md.d/no-agent-teardown.md) ~tmux-topology ~close-family-fakes ~transport-test-reconciling
