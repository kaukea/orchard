...
- `feature · todo · critical · blocked-on-answers · process ·` [Observability: a distributed, loosely coupled fleet you can actually watch — LAST ATTEMPT](TODO.md.d/observability.md) ~message-bus ~courier-delegating ~fleet-sidebar
  ...
  - `feature · todo · critical · plan-ready · process ·` [The sidebar: no AI in it, every project/feature/task/subtask/metric in real time](TODO.md.d/sidebar-teamwork.md) ~sidebar-polish ~sidebar-empty-rows ~fleet-sidebar
  - `feature · todo · critical · queued · process ·` [Close on the lifecycle event: the creator listens for stopped and cleans up — no agent teardown at all (Decisions 114+129)](TODO.md.d/no-agent-teardown.md) ~tmux-topology ~close-family-fakes
  - `bug · todo · critical · queued · process ·` [The question broker is built, tested and NOT RUNNING — every agent's ask path is dead](TODO.md.d/question-broker-dead.md) ~close-family-fakes ~operator-interacting
