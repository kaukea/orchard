...
those painters. The one-column indent glyph (THIRD on FOURTH), a plain
background fill for a row that has one, the open step's own FIFTH-toned
block background (`_open_block_bg`, per-task, unchanged since Decision-110),
and the SELECTED-row colour lift used instead of `curses.A_REVERSE`
(Decision-111 found `A_REVERSE`'s swap did too little work on this file's
own truecolor bands).
"""
from __future__ import annotations

from sidebar_colour import MUTED, WHITE, _CONTRAST_MIN_MARK, ensure_contrast, lerp  # noqa: E402
from sidebar_colour_lineage import content_colour_base, open_stage_colour  # noqa: E402
